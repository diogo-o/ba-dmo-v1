...
Quando o Destino é `Fabricação`, sugerir as Máquinas/Linhas. Quando é `Reparação`, sugerir reparadores/empresas como `Moldin`, `Intermolde` e nomes de operadores como `Orlando`, permitindo escrever outro. Quando é `Outros`, o utilizador escreve a descrição necessária.
